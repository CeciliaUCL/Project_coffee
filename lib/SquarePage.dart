...
class _SquarePageState extends State<SquarePage> {
  ...
  bool _nextEnabled = false;  // Added the state for the Next button
  List<Color> _pointColors = [Colors.grey, Colors.grey, Colors.grey, Colors.grey];
  Timer? _blinkTimer;
  ...
  // Send a start request to the backend and pass command '4'
  Future<void> _startHardwareControl() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await sendInputToServer('4');  // Sending '4' command
      } else {
  ...
  Future<bool> _onWillPop() async {
    ...
    return true;  // Allow the page to return
  }
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      onWillPop: _onWillPop,  // Intercept the back operation to ensure the program is stopped on exit
      child: Scaffold(
        backgroundColor: Colors.white,
        appBar: AppBar(
          title: Text('Square Page', style: TextStyle(color: Colors.black)),
          backgroundColor: Colors.white,
          elevation: 0,
        ...
        body: Center(
          child: Column(
            ...
            children: [
              CustomPaint(
                size: Size(120, 120),
                painter: SquareVerticesPainter(pointColors: _pointColors),
              ),
              SizedBox(height: 40),
  ...
  // Function to display status messages in a styled container
  Widget _buildStatusMessage(String message) {
    if (message.isEmpty) {
      return Container();  // Do not display if the message is empty
    }
    return Container(
...
class SquareVerticesPainter extends CustomPainter {
  ...
  SquareVerticesPainter({this.pointColors = const [Colors.grey, Colors.grey, Colors.grey, Colors.grey]});
