...
class _HexagonPageState extends State<HexagonPage> {
  ...
  // Send a start request to the backend and pass command '6'
  Future<void> _startHardwareControl() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        await sendInputToServer('6');  // Sending 'T' command
      } else {
  ...
  // Send a termination command when the user navigates back to the previous page
  ...
  Widget build(BuildContext context) {
    return WillPopScope(
      ...
      child: Scaffold(
        ...
        appBar: AppBar(
          title: Text('Hexagon Page', style: TextStyle(color: Colors.black)),
          backgroundColor: Colors.white,
          elevation: 0,
        ...
        body: Center(
          child: Column(
            ...
            children: [
              CustomPaint(
                size: Size(120, 120),
                painter: HexagonVerticesPainter(pointColors: _pointColors),
              ),
              SizedBox(height: 40),
...
class HexagonVerticesPainter extends CustomPainter {
  ...
  HexagonVerticesPainter({this.pointColors = const [Colors.grey, Colors.grey, Colors.grey, Colors.grey, Colors.grey, Colors.grey]});
